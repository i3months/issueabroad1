...
public interface ArticleRepository extends JpaRepository<Article, Long> {
    ...
    @Query("select a, r from Article a left join Reply r on r.article = a where a.ano = :ano")
    List<Object[]> getArticleWithReply(@Param("ano") Long ano);

    @Query(value = "select a, w, count(r) " +
            " from Article a " +
            " left join a.writer w " +
            " left join Reply r on r.article = a " +
            " group by a",
    ...
    @Query("select a, w, count(r) " +
            ...
            " left outer join Reply r on r.article = a " +
            " where a.ano = :ano")
